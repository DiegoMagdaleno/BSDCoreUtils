...
static unsigned int read_pairs (FILE *, struct ohash *, int, const char *,
                                unsigned int, int);
...
#endif
static unsigned int read_hints (FILE *, struct ohash *, int, const char *,
                                unsigned int);
...
static unsigned int traverse_node (struct node *, unsigned int,
                                   struct array *);
...
static int quiet_flag, long_flag, warn_flag, hints_flag, verbose_flag;
      ...
      for (;;)
        {
          ...
          if (str == sentinel)
            break;
          for (e = str; e < sentinel && !isspace ((unsigned char)*e); e++)
            continue;
          if (toggle)
            {
      ...
      for (;;)
        {
          ...
          if (a->order != NO_ORDER)
            {
              if (!quiet)
                warnx ("duplicate node %s in hints file %s", a->k, name);
            }
          else
            a->order = order++;
          str = e;
...
split_nodes (struct ohash *hash, struct array *heap, struct array *remaining)
{
  ...
  heap->t = ereallocarray (NULL, ohash_entries (hash), sizeof (struct node *));
  remaining->t
      = ereallocarray (NULL, ohash_entries (hash), sizeof (struct node *));
  heap->entries = 0;
  remaining->entries = 0;
...
parse_args (int argc, char *argv[], struct ohash *pairs)
{
  ...
  reverse_flag = quiet_flag = long_flag = warn_flag = hints_flag = verbose_flag
      = 0;
...
tsort (struct ohash *pairs)
{
      ...
      while (aux.entries)
        {
          ...
          for (l = n->arcs; l != NULL; l = l->next)
            if (l->node->refs != 0 && --l->node->refs == 0)
              {
                ENQUEUE (&aux, l->node);
              }
        }
      ...
      if (left != 0)
        {
          ...
          if (verbose_flag)
            warnx ("%u edge%s broken", n->refs, plural (n->refs));
          broken_arcs += n->refs;
          n->refs = 0;
  ...
  if (verbose_flag && broken_cycles != 0)
    warnx ("%u cycle%s broken, for a total of %u edge%s", broken_cycles,
           plural (broken_cycles), broken_arcs, plural (broken_arcs));
  if (warn_flag)
